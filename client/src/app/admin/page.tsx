    ...
    <div className="flex min-h-screen bg-neutral-50">
      ...
      <aside className="hidden w-60 shrink-0 flex-col justify-between border-r border-neutral-100 bg-surface-white px-4 py-6 lg:flex">
        <div>
          <div className="mb-8 flex items-center gap-2.5 px-2">
            ...
            <span className="text-[14px] font-semibold leading-[1.3] tracking-[-0.005em] text-neutral-900">
              Edificio Admin
            </span>
          </div>
          ...
          <nav className="flex flex-col gap-0.5">
            {navItems.map((item) => (
              <a
                key={item.label}
                href="#"
                className={`flex h-9 items-center rounded-lg px-3 text-[13px] font-medium leading-[1.4] transition-colors ${
                  item.active
        ...
        <div className="flex items-center gap-2.5 rounded-lg border border-neutral-100 px-2.5 py-2">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[11px] leading-[1.3] tracking-[0.01em] text-neutral-500">
              Administrador
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col">
        ...
        <header className="flex h-16 items-center justify-between border-b border-neutral-100 bg-surface-white px-6">
          <h1 className="text-[20px] font-bold leading-[1.2] tracking-[-0.015em] text-neutral-900">
            Panel principal
          </h1>
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {kpis.map((kpi) => (
              <div key={kpi.label} className="rounded-2xl border border-neutral-100 bg-surface-white p-5">
                <p className="text-[11px] font-medium uppercase leading-[1.3] tracking-[0.01em] text-neutral-500">
                  {kpi.label}
                </p>
                <p className="mt-2 text-[26px] font-bold leading-[1.2] tracking-[-0.015em] text-neutral-900 tabular-nums">
                  {kpi.value}
                </p>
                <p
                  className={`mt-1 text-[12px] font-medium leading-[1.3] ${
          ...
          <div className="mt-6 rounded-2xl border border-neutral-100 bg-surface-white">
            <div className="flex items-center justify-between border-b border-neutral-100 px-5 py-4">
              <h2 className="text-[15px] font-semibold leading-[1.3] tracking-[-0.005em] text-neutral-900">
                Pagos recientes
              </h2>
              <a
                href="#"
                className="text-[12px] font-medium leading-[1.3] tracking-[0.01em] text-primary-600 hover:text-primary-700"
              >
                Ver todos
              </a>
            </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-neutral-100">
                    <th className="px-5 py-2.5 text-left text-[11px] font-medium uppercase leading-[1.3] tracking-[0.01em] text-neutral-500">
                      Residente
                    </th>
                    <th className="px-5 py-2.5 text-left text-[11px] font-medium uppercase leading-[1.3] tracking-[0.01em] text-neutral-500">
                      Unidad
                  ...
                  {recentPayments.map((row) => (
                    <tr key={row.resident} className="border-b border-neutral-100 last:border-0">
                      ...
                      <td className="px-5 py-3">
                        <span
                          className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium leading-[1.3] ${statusStyles[row.status]}`}
                        >
                          {row.status}
                        </span>
                      </td>
                      <td className="px-5 py-3 text-[12px] leading-[1.3] tracking-[0.01em] text-neutral-500">
                        {row.date}
                      </td>
                    </tr>
                  ))}
